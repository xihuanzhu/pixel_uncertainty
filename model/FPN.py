...
class FPN(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes, back_bone='resnet', pretrained=True, isTrained=True):
    ...
    def forward(self, x):
        ...
        # Top-down
        p5 = self.toplayer(c5)
        p5 = self.conv2_drop(p5)
        p4 = self._upsample_add(p5, self.latlayer1(c4))
        p4 = self.conv2_drop(p4)
        p3 = self._upsample_add(p4, self.latlayer2(c3))
        p3 = self.conv2_drop(p3)
        p2 = self._upsample_add(p3, self.latlayer3(c2))
        p2 = self.conv2_drop(p2)
       # print("p2 shape ", p2.shape, "p3 shape ", p3.shape, "p4 shape ", p4.shape, "p5 shape ", p5.shape)
        ...
        t2 = self._upsample(self.conv3(s2), 4 * h, 4 * w)
        t3 = self._upsample(self.conv3(s3), 4 * h, 4 * w)
        t4 = self._upsample(self.conv3(s4), 4 * h, 4 * w)
        t5 = self._upsample(self.conv3(s5), 4 * h, 4 * w)
        #print("t2 shape ", t2.shape, "t3 shape ", t3.shape, "t4 shape ", t4.shape, "t5 shape ", t5.shape)
        if self.isTrained:
            #print("s2 shape ", s2.shape, "s3 shape ", s3.shape, "s4 shape ", s4.shape, "s5 shape ", s5.shape)
            #return self._upsample(self.conv3(s2 + s3 + s4 + s5), 4 * h, 4 * w)
            #t1 = (t2 + t3 + t4)/3.0
            
            
#             h1 = torch.cat((torch.unsqueeze(t2, dim=0), torch.unsqueeze(t3, dim=0)))
#             h1 = torch.cat((h1, torch.unsqueeze(t4, dim=0)))
#             #print(h1.shape)
#             return [t1, torch.var(h1, dim=0)]
...
if __name__ == "__main__":
    model = FPN([2,4,23,3], 32, back_bone="resnet50")
